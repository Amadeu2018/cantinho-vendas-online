...
-- 5. Prevent privilege escalation - make role only updatable by admins
CREATE POLICY "Only admins can change user roles" 
ON public.profiles 
FOR UPDATE 
USING (
  CASE 
    WHEN OLD.role = NEW.role THEN true  -- Allow other profile updates
    WHEN EXISTS (
      SELECT 1 FROM public.profiles 
      WHERE id = auth.uid() AND role = 'admin'
    ) THEN true  -- Allow admins to change roles
    ELSE false  -- Prevent non-admins from changing roles
  END
);
