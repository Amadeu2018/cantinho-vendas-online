# PLANO DE NEGÓCIOS
## Cantinho Algarvio - Restaurante e Delivery

---

**Versão:** 1.1  
**Data:** Dezembro 2025  
**Classificação:** Confidencial  
**URL de Produção:** [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org)

---

## ÍNDICE

1. [Sumário Executivo](#1-sumário-executivo)
2. [Descrição da Empresa](#2-descrição-da-empresa)
3. [Análise de Mercado](#3-análise-de-mercado)
4. [Produtos e Serviços](#4-produtos-e-serviços)
5. [Plano de Marketing](#5-plano-de-marketing)
6. [Plano Operacional](#6-plano-operacional)
7. [Estrutura Organizacional](#7-estrutura-organizacional)
8. [Plano Financeiro](#8-plano-financeiro)
9. [Análise de Viabilidade](#9-análise-de-viabilidade)
10. [Cronograma de Implementação](#10-cronograma-de-implementação)
11. [Anexos](#11-anexos)

---

## 1. SUMÁRIO EXECUTIVO

### 1.1 Visão Geral do Negócio

O **Cantinho Algarvio** é uma plataforma digital completa de delivery e gestão para restaurante, especializada em culinária angolana e portuguesa. O sistema oferece uma experiência integrada tanto para clientes quanto para administradores, permitindo desde a navegação no menu digital até a gestão completa de operações.

**Plataforma Online:** [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org)

### 1.2 Missão

Proporcionar a melhor experiência gastronómica angolana e portuguesa através de uma plataforma digital moderna, eficiente e acessível, conectando a autenticidade da culinária tradicional com a conveniência da tecnologia.

### 1.3 Visão

Tornar-se a principal referência em delivery de comida tradicional angolana e portuguesa em Luanda, expandindo para outras províncias de Angola até 2027.

### 1.4 Valores

- **Autenticidade**: Preservação das receitas e sabores tradicionais
- **Qualidade**: Ingredientes frescos e preparação cuidadosa
- **Inovação**: Tecnologia ao serviço da tradição
- **Compromisso**: Entregas pontuais e serviço excepcional
- **Sustentabilidade**: Práticas responsáveis e embalagens eco-friendly

### 1.5 Estado da Plataforma

> **⚠️ NOTA IMPORTANTE:** A plataforma encontra-se **operacional e funcional**, disponível em [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org). O sistema está em constante desenvolvimento e sujeito a **actualizações e melhorias contínuas** para optimizar a experiência do utilizador e as funcionalidades oferecidas.

### 1.6 Indicadores de Capacidade da Plataforma

| Indicador | Capacidade Técnica |
|-----------|-------------------|
| **Produtos no Catálogo** | Até 500 produtos |
| **Categorias Suportadas** | Ilimitadas |
| **Zonas de Entrega** | Configuráveis (actualmente 6 definidas) |
| **Métodos de Pagamento** | 4 métodos integrados |
| **Utilizadores Simultâneos** | Até 10.000 |
| **Pedidos por Hora** | Até 500 |

### 1.7 Metas de Lançamento (Primeiro Ano)

| Meta | Objectivo | Prazo |
|------|-----------|-------|
| Clientes Registados | 500 | 6 meses |
| Pedidos Mensais | 300 | 6 meses |
| Clientes Registados | 2.000 | 12 meses |
| Pedidos Mensais | 1.000 | 12 meses |
| Ticket Médio | 15.000 - 25.000 AOA | Contínuo |
| Taxa de Retenção | >60% | 12 meses |

---

## 2. DESCRIÇÃO DA EMPRESA

### 2.1 Informações Legais

| Campo | Informação |
|-------|------------|
| **Nome Comercial** | Cantinho Algarvio |
| **Website** | [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org) |
| **Localização** | Luanda, Angola |
| **Moeda Operacional** | AOA (Kwanza Angolano) |
| **Idioma Principal** | Português |
| **Fuso Horário** | Africa/Luanda (WAT) |
| **Regime Fiscal** | A definir |

### 2.2 Histórico

O Cantinho Algarvio nasceu da visão de unir a gastronomia tradicional portuguesa do Algarve com os sabores autênticos de Angola, criando uma experiência culinária única que celebra as raízes culturais partilhadas entre os dois países.

### 2.3 Estado Atual do Projeto

| Fase | Status | Descrição |
|------|--------|-----------|
| **Desenvolvimento** | ✅ Concluído | Plataforma completa desenvolvida |
| **Testes** | ✅ Em curso | Validação de funcionalidades |
| **Soft Launch** | 🔄 Próximo | Lançamento limitado para beta testers |
| **Lançamento Oficial** | ⏳ Planeado | Abertura ao público geral |

### 2.4 Modelo de Negócio

```
┌─────────────────────────────────────────────────────────────┐
│                    MODELO DE NEGÓCIO                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
│  │   CLIENTE   │───▶│  PLATAFORMA │───▶│  COZINHA    │     │
│  │   (App/Web) │    │   DIGITAL   │    │             │     │
│  └─────────────┘    └─────────────┘    └─────────────┘     │
│         │                  │                  │             │
│         ▼                  ▼                  ▼             │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
│  │   PEDIDO    │    │   GESTÃO    │    │  PREPARAÇÃO │     │
│  │             │    │   ADMIN     │    │             │     │
│  └─────────────┘    └─────────────┘    └─────────────┘     │
│         │                                    │             │
│         ▼                                    ▼             │
│  ┌─────────────┐                      ┌─────────────┐     │
│  │  PAGAMENTO  │                      │   ENTREGA   │     │
│  │             │                      │             │     │
│  └─────────────┘                      └─────────────┘     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 2.5 Fontes de Receita

1. **Venda de Refeições** (Principal)
   - Pratos individuais
   - Combos e menus
   - Sobremesas e bebidas

2. **Taxas de Entrega**
   - Variável por zona (1.000 - 3.000 AOA)

3. **Serviços de Catering**
   - Eventos corporativos
   - Casamentos e festas
   - Reuniões empresariais

4. **Eventos Especiais**
   - Pacotes pré-configurados
   - Personalização de menus

---

## 3. ANÁLISE DE MERCADO

### 3.1 Contexto do Mercado Angolano

#### 3.1.1 Panorama Económico

- **População de Luanda**: ~8,3 milhões de habitantes
- **Crescimento do setor de food delivery**: 25-30% anual
- **Penetração de smartphones**: ~45% da população
- **Acesso à internet**: Em crescimento constante

#### 3.1.2 Tendências do Setor

1. **Digitalização Acelerada**: Aumento da procura por soluções digitais pós-pandemia
2. **Conveniência**: Clientes valorizam rapidez e facilidade
3. **Qualidade vs Preço**: Mercado polarizado entre premium e económico
4. **Culinária Tradicional**: Valorização crescente das receitas locais

### 3.2 Análise SWOT

#### Forças (Strengths)
- ✅ Plataforma tecnológica robusta e moderna
- ✅ Cardápio autêntico angolano-português
- ✅ Sistema de gestão integrado
- ✅ Múltiplas formas de pagamento (Multicaixa, Transferência, Dinheiro)
- ✅ Cobertura planejada de 6+ zonas em Luanda

#### Fraquezas (Weaknesses)
- ⚠️ Marca nova no mercado (reconhecimento a construir)
- ⚠️ Operação ainda não iniciada
- ⚠️ Necessidade de validação do modelo com clientes reais
- ⚠️ Dependência de entregas próprias

#### Oportunidades (Opportunities)
- 🚀 Mercado de delivery em expansão
- 🚀 Baixa concorrência digital no segmento tradicional
- 🚀 Potencial de expansão para outras províncias
- 🚀 Parcerias com empresas para catering corporativo

#### Ameaças (Threats)
- ⚡ Entrada de grandes players internacionais
- ⚡ Instabilidade económica e cambial
- ⚡ Infraestrutura de internet variável
- ⚡ Custos de logística crescentes

### 3.3 Análise da Concorrência

| Concorrente | Pontos Fortes | Pontos Fracos | Posicionamento |
|-------------|---------------|---------------|----------------|
| **Tupuca** | Grande rede, marketing | Comida genérica | Mass market |
| **Ezimo** | Variedade | Qualidade inconsistente | Económico |
| **Restaurantes Locais** | Sabor autêntico | Sem delivery digital | Tradicional |
| **Cantinho Algarvio** | Tech + Tradição | Marca nova | Premium local |

### 3.4 Segmentação de Mercado

#### 3.4.1 Público-Alvo Primário (B2C)

- **Demografia**: Adultos 25-55 anos
- **Renda**: Classe média-alta
- **Localização**: Zonas urbanas de Luanda
- **Comportamento**: 
  - Valorizam qualidade sobre preço
  - Utilizam smartphones diariamente
  - Preferem comida tradicional
  - Fazem pedidos 2-4 vezes/mês

#### 3.4.2 Público-Alvo Secundário (B2B)

- **Empresas**: Escritórios e corporações
- **Eventos**: Organizadores de eventos privados
- **Hotéis**: Parcerias para serviço de quarto
- **Embaixadas**: Eventos diplomáticos

---

## 4. PRODUTOS E SERVIÇOS

### 4.1 Catálogo de Produtos (Planejado)

> **Nota:** O catálogo será expandido antes do lançamento oficial.

#### 4.1.1 Categorias Previstas

| Categoria | Nº Produtos Previsto | Faixa de Preço (AOA) |
|-----------|----------------------|----------------------|
| Pratos Principais | 15-20 | 8.000 - 25.000 |
| Entradas | 8-10 | 3.000 - 7.500 |
| Sobremesas | 6-8 | 2.500 - 6.000 |
| Bebidas | 10-15 | 500 - 5.000 |
| Carnes Grelhadas | 8-12 | 7.000 - 20.000 |
| Combos/Menus | 6-10 | 10.000 - 25.000 |
| Acompanhamentos | 8-10 | 1.500 - 4.000 |
| **TOTAL ESTIMADO** | **60-85** | **500 - 25.000** |

#### 4.1.2 Exemplos de Produtos (Cardápio Preliminar)

| Produto | Preço Sugerido (AOA) | Descrição |
|---------|----------------------|-----------|
| Combo Família 4 Pessoas | 15.000 - 20.000 | Picanha, frango, linguiça, acompanhamentos |
| Bifana no Prato | 8.500 - 10.000 | Bifana temperada com batata, arroz e ovo |
| Picanha Premium | 8.000 - 12.000 | Picanha bovina de primeira qualidade grelhada |
| Galinhada com Funge | 7.500 - 9.000 | Prato tradicional angolano completo |
| Mufete de Peixe | 7.000 - 9.000 | Prato típico angolano |
| Favas Guisadas | 5.500 - 7.000 | Entrada tradicional portuguesa |
| Bifanas à Moda do Porto | 5.000 - 6.500 | Receita tradicional do Porto |

#### 4.1.3 Faixa de Preços Planejada

- **Preço Mínimo**: ~2.500 AOA (sobremesas simples)
- **Preço Máximo**: ~25.000 AOA (combos família)
- **Ticket Médio Esperado**: 15.000 - 20.000 AOA

### 4.2 Zonas de Entrega

| Zona | Taxa (AOA) | Tempo Estimado |
|------|------------|----------------|
| Bairro Azul | 1.000 | 20-30 min |
| Maculusso | 1.500 | 25-35 min |
| Maianga | 1.500 | 25-35 min |
| Miramar | 1.800 | 30-40 min |
| Talatona | 2.500 | 35-50 min |
| Kilamba | 3.000 | 40-60 min |

> **Expansão Prevista:** Novas zonas serão adicionadas conforme demanda.

### 4.3 Métodos de Pagamento

1. **Dinheiro** (Cash on Delivery)
2. **Multicaixa Express** (Pagamento móvel)
3. **Transferência Bancária** (IBAN)
4. **Cartões Internacionais** (Stripe - em implementação)

### 4.4 Serviços de Catering e Eventos

#### Tipos de Eventos Suportados
- Casamentos
- Aniversários
- Eventos Corporativos
- Conferências
- Reuniões de Negócios
- Festas Privadas

#### Processo de Solicitação
1. Cliente preenche formulário online
2. Equipa analisa requisitos
3. Proposta de orçamento enviada
4. Aprovação do cliente
5. Pagamento de sinal (30-50%)
6. Execução do evento
7. Pagamento final

---

## 5. PLANO DE MARKETING

### 5.1 Estratégia de Posicionamento

**Posicionamento**: "Sabores autênticos de Angola e Portugal, entregues à sua porta com a conveniência da tecnologia moderna."

**Proposta de Valor Única (UVP)**: Combinação única de gastronomia tradicional angolana-portuguesa com plataforma digital de última geração.

### 5.2 Mix de Marketing (4Ps)

#### 5.2.1 Produto
- Menu diversificado com pratos autênticos
- Opções para diferentes ocasiões (individual, família, eventos)
- Personalização disponível para eventos
- Embalagens de qualidade para manter temperatura

#### 5.2.2 Preço
- **Estratégia**: Preço premium justificado pela qualidade
- **Ticket Médio Esperado**: 15.000 - 20.000 AOA
- **Promoções**: Combos familiares, descontos primeira compra
- **Programa de Fidelidade**: Planejado

#### 5.2.3 Praça (Distribuição)
- Plataforma web responsiva (PWA): [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org)
- Delivery próprio em 6 zonas de Luanda
- Tempo médio de entrega: 25-40 minutos
- Rastreamento em tempo real

#### 5.2.4 Promoção
- Marketing digital (redes sociais)
- Parcerias com influenciadores locais
- Programa de indicação
- Email marketing para base de clientes
- Presença em eventos gastronómicos

### 5.3 Estratégias de Aquisição de Clientes

#### Fase 1: Lançamento (Meses 1-3)
- Campanha "Primeira Compra" com 20% desconto
- Parcerias com 5 micro-influenciadores
- Presença ativa em Instagram e Facebook
- Google My Business optimizado

#### Fase 2: Crescimento (Meses 4-6)
- Programa de referência "Indique e Ganhe"
- Parcerias com empresas locais (catering)
- Campanhas sazonais (feriados angolanos)
- Reviews e testemunhos de clientes

#### Fase 3: Consolidação (Meses 7-12)
- Programa de fidelidade com pontos
- App nativo (iOS/Android) - Avaliação
- Expansão para novas zonas
- Parcerias com hotéis e embaixadas

### 5.4 Orçamento de Marketing (Estimativa Anual)

| Canal | Investimento Mensal (AOA) | Investimento Anual (AOA) |
|-------|---------------------------|--------------------------|
| Redes Sociais (Ads) | 150.000 | 1.800.000 |
| Influenciadores | 100.000 | 1.200.000 |
| Google Ads | 80.000 | 960.000 |
| Material Promocional | 50.000 | 600.000 |
| Eventos | 70.000 | 840.000 |
| **TOTAL** | **450.000** | **5.400.000** |

---

## 6. PLANO OPERACIONAL

### 6.1 Infraestrutura Tecnológica

#### 6.1.1 Stack Tecnológico

| Camada | Tecnologia | Função |
|--------|------------|--------|
| **Frontend** | React 18 + TypeScript | Interface do utilizador |
| **Estilização** | Tailwind CSS | Design system |
| **UI Components** | Shadcn/ui + Radix UI | Componentes reutilizáveis |
| **Backend** | Supabase | BaaS completo |
| **Base de Dados** | PostgreSQL | Armazenamento de dados |
| **Autenticação** | Supabase Auth | Gestão de utilizadores |
| **Storage** | Supabase Storage | Imagens de produtos |
| **Pagamentos** | Stripe + Multicaixa | Processamento financeiro |
| **Gráficos** | Recharts | Visualização de dados |

#### 6.1.2 Arquitetura do Sistema

```
┌─────────────────────────────────────────────────────────────────┐
│                        ARQUITETURA                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   ┌─────────────┐         ┌─────────────┐                      │
│   │   Cliente   │         │    Admin    │                      │
│   │   (React)   │         │   (React)   │                      │
│   └──────┬──────┘         └──────┬──────┘                      │
│          │                       │                              │
│          ▼                       ▼                              │
│   ┌─────────────────────────────────────────┐                  │
│   │            Supabase Client              │                  │
│   │         (API REST + Realtime)           │                  │
│   └──────────────────┬──────────────────────┘                  │
│                      │                                          │
│                      ▼                                          │
│   ┌─────────────────────────────────────────┐                  │
│   │              Supabase                    │                  │
│   │  ┌─────────┐ ┌─────────┐ ┌─────────┐   │                  │
│   │  │   DB    │ │  Auth   │ │ Storage │   │                  │
│   │  │(Postgres)│ │         │ │         │   │                  │
│   │  └─────────┘ └─────────┘ └─────────┘   │                  │
│   └─────────────────────────────────────────┘                  │
│                                                                 │
│   ┌─────────────────────────────────────────┐                  │
│   │          Integrações Externas           │                  │
│   │  ┌─────────┐ ┌─────────┐               │                  │
│   │  │ Stripe  │ │Multicaixa│               │                  │
│   │  └─────────┘ └─────────┘               │                  │
│   └─────────────────────────────────────────┘                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 6.2 Fluxo Operacional

#### 6.2.1 Processo de Pedido

```
Cliente                Sistema               Cozinha              Entrega
   │                      │                     │                    │
   │──── Navega Menu ────▶│                     │                    │
   │                      │                     │                    │
   │──── Adiciona ao ────▶│                     │                    │
   │     Carrinho         │                     │                    │
   │                      │                     │                    │
   │──── Checkout ───────▶│                     │                    │
   │                      │                     │                    │
   │──── Pagamento ──────▶│                     │                    │
   │                      │                     │                    │
   │                      │─── Novo Pedido ────▶│                    │
   │                      │                     │                    │
   │◀─── Confirmação ─────│                     │                    │
   │                      │                     │                    │
   │                      │                     │──── Preparo ──────▶│
   │                      │                     │                    │
   │◀─── Status Update ───│◀── Pronto ─────────│                    │
   │                      │                     │                    │
   │                      │                     │                    │
   │◀─── Rastreamento ────│◀────────────────────│◀── Em Rota ───────│
   │                      │                     │                    │
   │◀─── Entregue ────────│                     │                    │
   │                      │                     │                    │
```

#### 6.2.2 Estados do Pedido

| Estado | Descrição | Ação Necessária |
|--------|-----------|-----------------|
| `pending` | Pedido criado, aguardando pagamento | Confirmar pagamento |
| `confirmed` | Pagamento confirmado | Iniciar preparação |
| `preparing` | Em preparação na cozinha | Monitorar tempo |
| `ready` | Pronto para entrega | Despachar entregador |
| `delivering` | Em rota de entrega | Rastrear localização |
| `completed` | Entregue ao cliente | Solicitar avaliação |
| `cancelled` | Cancelado | Processar reembolso se aplicável |

### 6.3 Recursos Humanos Necessários

| Função | Quantidade | Responsabilidades |
|--------|------------|-------------------|
| Gerente Geral | 1 | Supervisão geral, estratégia |
| Chef Principal | 1 | Cardápio, qualidade, receitas |
| Cozinheiros | 2-3 | Preparação dos pratos |
| Atendimento/Caixa | 1 | Pedidos, pagamentos |
| Entregadores | 3-5 | Entregas nas zonas cobertas |
| Marketing Digital | 1 | Redes sociais, campanhas |
| Suporte Técnico | 1 (part-time) | Manutenção da plataforma |

### 6.4 Horário de Funcionamento (Previsto)

| Dia | Horário |
|-----|---------|
| Segunda a Quinta | 11:00 - 22:00 |
| Sexta e Sábado | 11:00 - 23:00 |
| Domingo | 12:00 - 21:00 |
| Feriados | 12:00 - 21:00 |

---

## 7. ESTRUTURA ORGANIZACIONAL

### 7.1 Organigrama

```
                    ┌─────────────────┐
                    │   Proprietário  │
                    │   /Investidor   │
                    └────────┬────────┘
                             │
                    ┌────────┴────────┐
                    │  Gerente Geral  │
                    └────────┬────────┘
                             │
        ┌────────────────────┼────────────────────┐
        │                    │                    │
┌───────┴───────┐   ┌───────┴───────┐   ┌───────┴───────┐
│   Operações   │   │   Comercial   │   │  Tecnologia   │
│               │   │               │   │               │
├───────────────┤   ├───────────────┤   ├───────────────┤
│ - Chef        │   │ - Marketing   │   │ - Suporte     │
│ - Cozinheiros │   │ - Atendimento │   │ - Manutenção  │
│ - Entregadores│   │ - Eventos     │   │               │
└───────────────┘   └───────────────┘   └───────────────┘
```

### 7.2 Perfis de Utilizador do Sistema

| Perfil | Descrição | Permissões |
|--------|-----------|------------|
| **Admin** | Administrador do sistema | Acesso total: dashboard, produtos, pedidos, finanças, configurações |
| **Customer** | Cliente da plataforma | Menu, carrinho, checkout, perfil, histórico, favoritos |
| **Seller** | Vendedor (futuro) | Gestão de produtos próprios, pedidos relacionados |

### 7.3 Papéis e Responsabilidades

#### Administrador
- Gestão completa do catálogo de produtos
- Processamento e acompanhamento de pedidos
- Análise de métricas e relatórios
- Configurações do sistema
- Gestão de utilizadores
- Gestão financeira e faturas

#### Cliente
- Navegação no menu
- Realização de pedidos
- Rastreamento de entregas
- Gestão de favoritos
- Avaliações de produtos
- Solicitação de eventos

---

## 8. PLANO FINANCEIRO

### 8.1 Investimento Inicial

#### 8.1.1 Equipamentos de Cozinha (Valores Reais - Faturas Proforma)

**Fornecedor: KOLLER** (Pró-forma n.º PP 2025/601 - Data: 27/12/2025)

| Código | Descrição | Preço Unitário (AOA) | Desconto | Total (AOA) |
|--------|-----------|----------------------|----------|-------------|
| 10360 | Mesa de trabalho | 350.877,19 | 0% | 350.877,19 |
| 10405 | Casquinha de gelado | 118.421,05 | 20% | 94.736,84 |
| 10335 | SXC-22 Moedor de carne | 480.263,16 | 20% | 384.210,53 |
| 10197 | Forno elétrico de 1 porta 2 tabuleiros | 657.785,09 | 20% | 526.228,07 |
| 10095 | BL-818 Chapa de hambúrguer elétrica pequena | 218.201,75 | 20% | 174.561,40 |
| 20010 | Fogão 6 bocas | 526.315,79 | 20% | 421.052,63 |
| 10363 | Lava loiça duas pias | 458.333,33 | 20% | 366.666,56 |
| 10379 | Exaustor 1,5m | 1.039.473,68 | 20% | 831.578,94 |
| | **Subtotal Líquido** | | | **3.149.912,27** |
| | **IVA (14%)** | | | **440.987,72** |
| | **TOTAL KOLLER** | | | **3.590.899,99** |

**Fornecedor: Mauena Comércio Geral LDA** (Factura Pró-Forma n.º 10112/2025 - Data: 27/12/2025)

| Artigo | Descrição | Qtd | Preço Unitário (AOA) | Total (AOA) |
|--------|-----------|-----|----------------------|-------------|
| 1.00 | KW50-2a (Equipamento Industrial) | 2 | 690.000,00 | 1.380.000,00 |
| | **TOTAL MAUENA** | | | **1.380.000,00** |

#### 8.1.2 Resumo do Investimento Inicial

| Categoria | Valor (AOA) | Observações |
|-----------|-------------|-------------|
| **Equipamentos KOLLER** | 3.590.899,99 | Fatura Pró-forma PP 2025/601 |
| **Equipamentos Mauena** | 1.380.000,00 | Fatura Pró-forma 10112/2025 |
| **Subtotal Equipamentos** | **4.970.899,99** | **Valores documentados** |
| Desenvolvimento da Plataforma | 5.000.000,00 | Já realizado |
| Mobiliário e Decoração | 1.500.000,00 | Estimativa |
| Veículos de Entrega | 2.000.000,00 | 2 motos + 1 carro |
| Capital de Giro Inicial | 2.500.000,00 | 3 meses de operação |
| Marketing de Lançamento | 1.000.000,00 | Campanha inicial |
| Licenças e Legalizações | 500.000,00 | Alvarás, registos |
| **Subtotal** | **17.470.899,99** | |
| Imprevistos (10%) | 1.747.090,00 | Reserva de contingência |
| **TOTAL INVESTIMENTO** | **19.217.989,99** | ~$19.200 USD |

> **Nota:** Os valores de equipamentos são baseados em faturas proforma reais datadas de 27/12/2025. Os demais valores são estimativas a serem validadas.

### 8.2 Estimativa de Custos Operacionais

> **⚠️ NOTA:** Os valores apresentados nesta secção são **estimativas indicativas** para efeitos do estudo de viabilidade. Os custos reais deverão ser validados e ajustados conforme a operação.

#### 8.2.1 Custos Diários Estimados

| Categoria | Valor Diário (AOA) | Observações |
|-----------|-------------------|-------------|
| Matéria-prima | 20.000 | Baseado em 40% da receita diária |
| Combustível entregas | 5.000 | Média para 6-10 entregas |
| Embalagens | 3.333 | Custo por entrega estimado |
| **TOTAL DIÁRIO** | **28.333** | **Dias úteis de operação** |

#### 8.2.2 Custos Mensais Estimados

| Categoria | Valor Mensal (AOA) | % do Total | Observações |
|-----------|-------------------|------------|-------------|
| **CUSTOS FIXOS** | | | |
| Aluguer | 400.000 | 16% | Espaço comercial |
| Salários (equipa básica) | 800.000 | 32% | 5-7 colaboradores |
| Utilities (água, luz, gás) | 150.000 | 6% | Média estimada |
| Internet + Telefone | 50.000 | 2% | Pacote empresarial |
| Seguros | 80.000 | 3% | Responsabilidade civil |
| Manutenção Plataforma | 100.000 | 4% | Suporte técnico |
| **Subtotal Fixos** | **1.580.000** | **63%** | |
| **CUSTOS VARIÁVEIS** | | | |
| Matéria-prima | 600.000 | 24% | 40% da receita |
| Embalagens | 100.000 | 4% | Por pedido |
| Combustível entregas | 150.000 | 6% | Motos e veículo |
| Taxas de pagamento | 30.000 | 1% | 2% das transações |
| Marketing | 50.000 | 2% | Redes sociais |
| **Subtotal Variáveis** | **930.000** | **37%** | |
| **TOTAL MENSAL** | **2.510.000** | **100%** | |

#### 8.2.3 Custos Anuais Estimados

| Categoria | Valor Anual (AOA) | Observações |
|-----------|------------------|-------------|
| Custos Fixos | 18.960.000 | 1.580.000 × 12 meses |
| Custos Variáveis | 11.160.000 | 930.000 × 12 meses |
| **TOTAL ANUAL** | **30.120.000** | **Estimativa ano 1** |

### 8.3 Projeção de Receitas (12 meses)

> **⚠️ NOTA IMPORTANTE:** As projeções de receitas apresentadas são **valores indicativos** baseados em estimativas de mercado e benchmarks do setor. Estes dados são apresentados exclusivamente para efeitos do **estudo de viabilidade** e deverão ser validados e ajustados com dados reais após o início das operações.

#### Receitas Diárias Estimadas

| Período | Pedidos/Dia | Ticket Médio (AOA) | Receita Diária (AOA) |
|---------|-------------|-------------------|---------------------|
| Mês 1-3 | 5-10 | 15.000-16.000 | 75.000 - 160.000 |
| Mês 4-6 | 12-18 | 16.000-17.000 | 192.000 - 306.000 |
| Mês 7-9 | 20-25 | 18.000-18.500 | 360.000 - 462.500 |
| Mês 10-12 | 27-35 | 19.000-20.000 | 513.000 - 700.000 |

#### Receitas Mensais Estimadas - Cenário Conservador

| Mês | Pedidos/Dia | Ticket Médio (AOA) | Receita Mensal (AOA) |
|-----|-------------|-------------------|---------------------|
| Mês 1 | 5 | 15.000 | 2.250.000 |
| Mês 2 | 7 | 15.000 | 3.150.000 |
| Mês 3 | 10 | 16.000 | 4.800.000 |
| Mês 4 | 12 | 16.000 | 5.760.000 |
| Mês 5 | 15 | 17.000 | 7.650.000 |
| Mês 6 | 18 | 17.000 | 9.180.000 |
| Mês 7 | 20 | 18.000 | 10.800.000 |
| Mês 8 | 22 | 18.000 | 11.880.000 |
| Mês 9 | 25 | 18.500 | 13.875.000 |
| Mês 10 | 27 | 19.000 | 15.390.000 |
| Mês 11 | 30 | 19.000 | 17.100.000 |
| Mês 12 | 35 | 20.000 | 21.000.000 |
| **TOTAL ANO 1** | | | **122.835.000** |

#### Receitas Anuais Estimadas

| Ano | Receita Anual (AOA) | Crescimento |
|-----|--------------------|-----------| 
| Ano 1 | 122.835.000 | - |
| Ano 2 | 184.252.500 | +50% |
| Ano 3 | 257.953.500 | +40% |

### 8.4 Ponto de Equilíbrio (Break-Even)

```
Custos Fixos Mensais: 1.580.000 AOA
Margem de Contribuição: 60% (após custos variáveis)

Ponto de Equilíbrio = Custos Fixos / Margem de Contribuição
Ponto de Equilíbrio = 1.580.000 / 0.60
Ponto de Equilíbrio = 2.633.333 AOA/mês

Com ticket médio de 15.000 AOA:
Pedidos necessários = 2.633.333 / 15.000 = 176 pedidos/mês
Pedidos por dia = 176 / 30 = 6 pedidos/dia
```

### 8.5 Indicadores Financeiros Projetados

> **⚠️ NOTA:** Estes indicadores são **projeções indicativas** para efeitos do estudo de viabilidade.

| Indicador | Ano 1 | Ano 2 | Ano 3 |
|-----------|-------|-------|-------|
| Receita Bruta (AOA) | 122.835.000 | 184.252.500 | 257.953.500 |
| Custos Totais (AOA) | 98.268.000 | 129.097.500 | 167.669.775 |
| Lucro Bruto (AOA) | 24.567.000 | 55.155.000 | 90.283.725 |
| Margem Bruta | 20% | 30% | 35% |
| ROI | 44% | 123% | 230% |

### 8.6 Resumo Financeiro - Despesas e Receitas

> **Quadro síntese para análise de viabilidade financeira**

| Período | Despesas Estimadas (AOA) | Receitas Estimadas (AOA) | Resultado (AOA) |
|---------|-------------------------|-------------------------|-----------------|
| **DIÁRIO** | 28.333 | 75.000 - 700.000 | Variável |
| **MENSAL** | 2.510.000 | 2.250.000 - 21.000.000 | Variável |
| **ANUAL (Ano 1)** | 30.120.000 | 122.835.000 | +92.715.000 |

### 8.7 Necessidade de Financiamento

Com base no investimento inicial e nos custos operacionais estimados, apresenta-se a seguinte análise de necessidade de financiamento:

| Item | Valor (AOA) | Observações |
|------|-------------|-------------|
| Investimento Inicial Total | 19.217.989,99 | Conforme secção 8.1 |
| Capital de Giro (3 meses) | 7.530.000 | 2.510.000 × 3 meses |
| **NECESSIDADE TOTAL** | **26.747.989,99** | **~$26.750 USD** |

> **Nota:** Recomenda-se que o pedido de financiamento seja acompanhado de uma **carta formal de solicitação de empréstimo** especificando o valor, prazo de retorno e garantias oferecidas (ver Anexo 11.5).

---

## 9. ANÁLISE DE VIABILIDADE

### 9.1 Viabilidade Técnica

| Critério | Status | Observações |
|----------|--------|-------------|
| Plataforma funcional | ✅ Implementada | React + Supabase operacional |
| URL de Produção | ✅ Activo | [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org) |
| Pagamentos integrados | ✅ Parcial | Multicaixa + Stripe configurados |
| Escalabilidade | ✅ Alta | Supabase suporta crescimento |
| Manutenção | ✅ Baixa | Arquitetura moderna e documentada |
| Segurança | ✅ Implementada | RLS, autenticação, logs |

### 9.2 Viabilidade Económica

| Critério | Valor | Avaliação |
|----------|-------|-----------|
| Investimento Inicial | 17.050.000 AOA | Moderado |
| Payback Estimado | 8-10 meses | Atrativo |
| ROI Ano 1 | 44% | Bom |
| Margem Bruta | 20-35% | Saudável |
| Break-even | 6 pedidos/dia | Alcançável |

### 9.3 Viabilidade Mercadológica

| Critério | Avaliação | Justificativa |
|----------|-----------|---------------|
| Tamanho do Mercado | Grande | 8M+ habitantes em Luanda |
| Crescimento | Alto | Setor delivery +25% anual |
| Concorrência | Moderada | Poucos focados em culinária tradicional |
| Diferenciação | Alta | Proposta única angolana-portuguesa |
| Barreiras de Entrada | Baixas | Requer validação constante |

### 9.4 Viabilidade Operacional

| Critério | Status | Plano de Ação |
|----------|--------|---------------|
| Plataforma | ✅ Pronta | [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org) |
| Equipa | A recrutar | Plano de contratação gradual |
| Fornecedores | A estabelecer | Parcerias com produtores locais |
| Logística | Definida | 6 zonas cobertas |
| Processos | Documentados | Sistema automatiza 70% |
| Escalabilidade | Alta | Plataforma permite crescimento |

### 9.5 Matriz de Riscos e Mitigação

| Risco | Probabilidade | Impacto | Mitigação |
|-------|---------------|---------|-----------|
| Falha técnica | Baixa | Alto | Backups diários, suporte técnico |
| Baixa adesão | Média | Alto | Marketing agressivo, promoções |
| Concorrência | Média | Médio | Diferenciação, qualidade |
| Custos elevados | Média | Médio | Controle rigoroso, fornecedores |
| Flutuação cambial | Alta | Médio | Preços em AOA, ajustes periódicos |

---

## 10. CRONOGRAMA DE IMPLEMENTAÇÃO

### 10.1 Fases do Projeto

```
2024                    2025                              2026
 Q4     Q1      Q2      Q3      Q4      Q1      Q2
 ├──────┼───────┼───────┼───────┼───────┼───────┼───────┤
 │      │       │       │       │       │       │       │
 │ DEV  │LAUNCH │GROWTH │GROWTH │EXPAND │SCALE  │SCALE  │
 │      │       │       │       │       │       │       │
 └──────┴───────┴───────┴───────┴───────┴───────┴───────┘
```

### 10.2 Marcos (Milestones)

| Fase | Período | Objetivos | KPIs |
|------|---------|-----------|------|
| **Desenvolvimento** | Q4 2024 | Plataforma completa | ✅ Concluído |
| **Lançamento** | Q1 2025 | Primeiros 50 clientes | 50 clientes, 100 pedidos |
| **Crescimento** | Q2-Q3 2025 | Expansão base clientes | 200 clientes, 500 pedidos |
| **Expansão** | Q4 2025 | Novas zonas, catering consolidado | 500 clientes, 1500 pedidos |
| **Escala** | 2026 | App nativo, novas cidades | 2000 clientes, 5000 pedidos |

### 10.3 Plano de Ação Imediato (90 dias)

| Semana | Ações |
|--------|-------|
| 1-2 | Completar catálogo (mínimo 30 produtos) |
| 3-4 | Recrutar equipa básica (chef + 2 cozinheiros) |
| 5-6 | Campanha de lançamento (redes sociais) |
| 7-8 | Parcerias com 10 empresas para catering |
| 9-10 | Otimização baseada em feedback |
| 11-12 | Avaliação e ajustes estratégicos |

---

## 11. ANEXOS

### 11.1 Glossário de Termos

| Termo | Definição |
|-------|-----------|
| **AOA** | Kwanza Angolano (moeda oficial) |
| **B2B** | Business to Business |
| **B2C** | Business to Consumer |
| **KPI** | Key Performance Indicator |
| **MVP** | Minimum Viable Product |
| **PWA** | Progressive Web Application |
| **RLS** | Row Level Security |
| **ROI** | Return on Investment |
| **SWOT** | Strengths, Weaknesses, Opportunities, Threats |
| **UVP** | Unique Value Proposition |

### 11.2 Recursos Online

| Recurso | URL |
|---------|-----|
| **Plataforma Principal** | [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org) |
| **Menu Digital** | [https://cantinhoalgarvio.org/menu](https://cantinhoalgarvio.org/menu) |
| **Sobre Nós** | [https://cantinhoalgarvio.org/about](https://cantinhoalgarvio.org/about) |
| **Contacto** | [https://cantinhoalgarvio.org/contact](https://cantinhoalgarvio.org/contact) |

### 11.3 Referências

1. Análise de Mercado - Estudos setoriais Angola 2024
2. Benchmarks de Delivery - Mercado africano 2024
3. Projeções económicas - FMI Angola 2025

### 11.4 Contactos

| Departamento | Contacto |
|--------------|----------|
| Geral | contato@cantinhoalgarvio.org |
| Suporte | suporte@cantinhoalgarvio.org |
| Eventos | eventos@cantinhoalgarvio.org |
| Comercial | comercial@cantinhoalgarvio.org |
| Telefone | +244 924 678 544 |

### 11.5 Modelo de Carta de Solicitação de Empréstimo

---

**[Local], [Data]**

**À**
**[Nome da Instituição Financeira]**
**Departamento de Crédito**

**Assunto:** Solicitação de Empréstimo para Implementação do Projecto "Cantinho Algarvio"

Exmos. Senhores,

Vimos, por este meio, solicitar a apreciação para concessão de **empréstimo no valor de [VALOR A DEFINIR] AOA** (equivalente a aproximadamente $[VALOR USD] USD), destinado à implementação e operacionalização do projecto de restaurante e delivery "**Cantinho Algarvio**".

**1. FINALIDADE DO EMPRÉSTIMO**
- Aquisição de equipamentos de cozinha industrial
- Capital de giro para os primeiros meses de operação
- Marketing e lançamento da plataforma

**2. VALOR SOLICITADO**
- Investimento Inicial: 19.217.989,99 AOA
- Capital de Giro: 7.530.000,00 AOA
- **Total Solicitado:** [A DEFINIR COM BASE NA DISPONIBILIDADE DO PROMOTOR]

**3. PRAZO DE RETORNO PROPOSTO**
- Prazo: [24/36/48] meses
- Carência: [6] meses
- Modalidade: Prestações mensais fixas

**4. GARANTIAS OFERECIDAS**
- [Descrever garantias reais: imóveis, equipamentos, etc.]
- [Descrever garantias pessoais: avales, fianças, etc.]
- [Descrever outras garantias: seguros, penhor de créditos, etc.]

**5. ANEXOS**
- Plano de Negócios completo
- Faturas proforma dos equipamentos
- Documentos de identificação do(s) promotor(es)
- Comprovativo de morada
- [Outros documentos relevantes]

Colocamo-nos à disposição para prestar quaisquer esclarecimentos adicionais.

Com os melhores cumprimentos,

___________________________________
**[Nome do Promotor/Representante Legal]**
**NIF:** [Número de Identificação Fiscal]
**Telefone:** +244 924 678 544
**Email:** contato@cantinhoalgarvio.org

---

## APROVAÇÕES

| Função | Nome | Assinatura | Data |
|--------|------|------------|------|
| Proprietário | _____________ | _____________ | ___/___/2025 |
| Gerente Geral | _____________ | _____________ | ___/___/2025 |
| Consultor Financeiro | _____________ | _____________ | ___/___/2025 |

---

**Documento preparado por:** Sistema Cantinho Algarvio  
**Versão:** 1.2  
**Última atualização:** Janeiro 2026  
**Próxima revisão:** Março 2026  
**URL de Produção:** [https://cantinhoalgarvio.org](https://cantinhoalgarvio.org)

---

*Este documento é confidencial e destinado exclusivamente para uso interno e avaliação de investidores. A reprodução ou distribuição não autorizada é proibida.*

> **⚠️ NOTA:** Os dados financeiros e operacionais apresentados são projeções e estimativas. Os dados reais de produção serão coletados após o lançamento oficial da plataforma.
