...
ADD COLUMN IF NOT EXISTS sale_unit TEXT DEFAULT 'unit' CHECK (sale_unit IN ('unit', 'kg', 'combo', 'rodizio')),
ADD COLUMN IF NOT EXISTS prep_time_minutes INTEGER DEFAULT 30,
ADD COLUMN IF NOT EXISTS meat_options JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS spice_level INTEGER DEFAULT 0 CHECK (spice_level >= 0 AND spice_level <= 5),
...
-- Criar tabela para promoções gerais (expandindo o sistema existente)
ALTER TABLE public.promotions 
ADD COLUMN IF NOT EXISTS promotion_type TEXT DEFAULT 'percentage' CHECK (promotion_type IN ('percentage', 'fixed_amount', 'combo')),
...
-- Políticas para grill_customizations
...
-- Inserir alguns produtos exemplo de churrascaria
